...
namespace GK {
	public class Fracture : MonoBehaviour {
        ...
        public float projectionImpulseForce;
		void Start() {
			if (InitialMesh) {
		...
		public void DoFracture(Vector3 center, Vector3 dirImpulse) {
			if (points.Count >= ClusterCount) {
				...
				DoFracture(clusters, dirImpulse);
			}
		}

        public void DoFracture(Vector3 transmitImpulse) {
			var clusters = new List<Vector3>(ClusterCount);
			...
			DoFracture(clusters, transmitImpulse);
		}


		void DoFracture(List<Vector3> clusters, Vector3 impulse) {
			...
			for (int i = 0; i < ClusterCount; i++) {
				if (clouds[i] == null) {
                    ...
                    children[i].GetComponent<Rigidbody>().AddForce(impulse, ForceMode.Impulse);
				}
			}
		}
	}
